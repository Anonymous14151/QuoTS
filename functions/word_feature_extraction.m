...
function [KEY_VECTOR1, KEY_VECTOR2, KEY_VECTOR3] = uni_dimensional_extraction(signal, win_size)
    ...
    keywords_vals1 = multi_scale_vector_generation(signal, win_size);
    keywords_vals2 = multi_scale_vector_generation(signal, fix(win_size/4));
    %window 2 = win_size/16
    if(win_size/4<2)
        win_size = 32;
    end
    keywords_vals3 = multi_scale_vector_generation(signal, fix(win_size/8));
...
function [KEY_VECTOR1, KEY_VECTOR2, KEY_VECTOR3] = multi_dimensional_extraction(X, win_size)
    ...
    for i = 1:nbr_signals
        ...
        %window 2 = win_size/4
        d2 = multi_d_scale_vector_generation(d2, s, i, fix(win_size/4));

        %window 3 = win_size/8
        d3 = multi_d_scale_vector_generation(d3, s, i, fix(win_size/8));
    end
...
function keywords_vals = multi_scale_vector_generation(signal, win_size)
